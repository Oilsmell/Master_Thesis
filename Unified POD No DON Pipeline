# ================================================================
# UNIFIED POD PIPELINE: MMD-GAN | MemAE | MK-MMD
# 세 모델 전체 학습/평가 → 각 모델 POD 계산 → 비교
# ================================================================
...
SAVE_DIR_COMP   = r"E:\2ndstructuredata\Unified_POD_Comparison"
...
print("=" * 70)
print("  UNIFIED POD PIPELINE: MMD-GAN | MemAE | MK-MMD")
...
for epoch in range(GAN_EPOCHS):
    for (real_data,) in gan_loader:
        ...
        opt_g.step()
    if (epoch+1) % 20 == 0:
        print(f"    Epoch {epoch+1}/{GAN_EPOCHS} | C: {c_loss.item():.6f} | G: {g_loss.item():.6f}")

# 가중치 저장
torch.save(gan_gen.state_dict(),    os.path.join(SAVE_DIR_GAN, "MMD_Generator.pth"))
torch.save(gan_critic.state_dict(), os.path.join(SAVE_DIR_GAN, "MMD_Critic.pth"))
...
# Synthetic threshold: synth DI 0.00 기반
...
# ================================================================
# [5] MK-MMD 학습 (Optuna) 및 평가
# ================================================================
print("\n" + "=" * 70)
print("  [5] MK-MMD: Optuna 최적화 및 POD 계산")
print("=" * 70)
...
svm_train_h_raw, ns_total = load_data_svm(os.path.join(DIR_B_RAW, FILE_B))
np.random.seed(42)
np.random.shuffle(svm_train_h_raw)
svm_split_idx   = int(ns_total * 0.7)
svm_train_h_raw = svm_train_h_raw[:svm_split_idx]
svm_val_h_raw   = svm_train_h_raw[svm_split_idx:] if svm_split_idx < len(svm_train_h_raw) else svm_train_h_raw[-int(ns_total*0.3):]

# Train/Val 다시 분리
all_h, _ = load_data_svm(os.path.join(DIR_B_RAW, FILE_B))
np.random.seed(42)
...
ax.set_title("Synthetic Damage: POD Comparison", fontweight='bold', fontsize=13)
